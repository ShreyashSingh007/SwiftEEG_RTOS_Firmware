...
def flagged(secs, taufn, mult=4.6):
    ...
    total = 0.0
    for s in secs:
        k = float(s[1]); d = min(k / 2, 2 / k)
        total += (4.96 + 3.04 * d * d) * taufn(s[0], s[1]) + 2.0
...
print("new rule: sum over sections of (4.96 + 3.04 d^2) tau + 2, d = min(k/2, 2/k), tau from the pole radius")
